...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    $where = [];
    $params = [];
    if ($dateFrom !== '') { $where[] = "s.date >= ?"; $params[] = dateDB($dateFrom); }
    if ($dateTo !== '') { $where[] = "s.date <= ?"; $params[] = dateDB($dateTo); }
    if ($partyId > 0) { $where[] = "s.party_id = ?"; $params[] = $partyId; }
    if (in_array($paymentStatus, ['paid','unpaid','partial'])) { $where[] = "s.payment_status = ?"; $params[] = $paymentStatus; }
    $whereSql = $where ? 'WHERE ' . implode(' AND ', $where) : '';

    $rows = fetchAll("SELECT s.date, s.invoice_no, p.name as party_name,
        (SELECT COUNT(*) FROM sale_items WHERE sale_id = s.id) as qty,
        s.subtotal, s.tax_amount, s.discount_amount, s.total, s.paid_amount, s.due_amount, s.payment_status
        FROM sales s LEFT JOIN parties p ON s.party_id = p.id $whereSql ORDER BY s.date ASC", $params);

    $csvData = [];
    foreach ($rows as $r) {
        $csvData[] = [$r['date'], $r['invoice_no'], $r['party_name'] ?: 'Walk-in', $r['qty'], $r['subtotal'], $r['tax_amount'], $r['discount_amount'], $r['total'], $r['paid_amount'], $r['due_amount'], ucfirst($r['payment_status'])];
    }
    exportCSV(['Date','Invoice No','Party','Qty','Subtotal','Tax','Discount','Total','Paid','Due','Status'], $csvData, 'sales_report');
}
...
// Sales list
$sales = fetchAll("SELECT s.*, p.name as party_name,
    (SELECT COUNT(*) FROM sale_items WHERE sale_id = s.id) as items_count
    FROM sales s LEFT JOIN parties p ON s.party_id = p.id
    $whereSql ORDER BY s.date DESC, s.id DESC
    LIMIT {$pagination['per_page']} OFFSET {$pagination['offset']}", $params);

// Chart data: daily sales trend
$chartSql = "SELECT s.date, SUM(s.total) as daily_total
    FROM sales s $whereSql GROUP BY s.date ORDER BY s.date ASC";
$chartData = fetchAll($chartSql, $params);
...
// Parties for filter
$parties = fetchAll("SELECT id, name FROM parties WHERE status = 1 ORDER BY name ASC");
    ...
    <div class="col-md">
        <div class="card border-0 shadow-sm">
            <div class="card-body text-center py-3">
                ...
                <h5 class="mb-0 text-info"><?= money($summary['total_discount']) ?></h5>
            </div>
        </div>
    </div>
    <div class="col-md">
        ...
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-2">
                ...
                <select name="party_id" class="form-select form-select-sm">
                    ...
                    <?php foreach ($parties as $p): ?>
                        <option value="<?= $p['id'] ?>" <?= $partyId == $p['id'] ? 'selected' : '' ?>><?= sanitize($p['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                ...
                <select name="group_by" class="form-select form-select-sm">
                    ...
                    <option value="party" <?= $groupBy === 'party' ? 'selected' : '' ?>>Party</option>
                </select>
            </div>
            <div class="col-md-2 d-flex gap-1">
                <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-filter me-1"></i>Filter</button>
                <a href="report_sales.php" class="btn btn-outline-secondary btn-sm">Reset</a>
            </div>
        </form>
...
<!-- Chart -->
<div class="card mb-4">
    <div class="card-header"><h6 class="mb-0">Sales Trend</h6></div>
    <div class="card-body">
...
<div class="card mb-4">
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            <tbody>
                <?php if (empty($sales)): ?>
                    <tr><td colspan="11" class="text-center text-muted py-4">No sales found for the selected filters.</td></tr>
                <?php else: ?>
                    <?php foreach ($sales as $s): ?>
                        <tr>
                            <td><?= dateFormatted($s['date']) ?></td>
                            <td><a href="sale_view.php?id=<?= $s['id'] ?>" class="fw-semibold text-decoration-none"><?= sanitize($s['invoice_no']) ?></a></td>
                            <td><?= sanitize($s['party_name'] ?? 'Walk-in') ?></td>
                            <td class="text-center"><?= intval($s['items_count']) ?></td>
                            <td class="text-end"><?= money($s['subtotal']) ?></td>
                            <td class="text-end"><?= money($s['tax_amount']) ?></td>
                            <td class="text-end"><?= money($s['discount_amount']) ?></td>
                            <td class="text-end fw-bold"><?= money($s['total']) ?></td>
                            <td class="text-end text-success"><?= money($s['paid_amount']) ?></td>
                            <td class="text-end text-danger"><?= money($s['due_amount']) ?></td>
                            <td>
            ...
            <tfoot class="table-light fw-bold">
                <tr>
                    ...
                    <td class="text-end"><?= money($summary['total_sales'] - $summary['total_tax'] - $summary['total_discount']) ?></td>
                    <td class="text-end"><?= money($summary['total_tax']) ?></td>
                    <td class="text-end"><?= money($summary['total_discount']) ?></td>
                    <td class="text-end"><?= money($summary['total_sales']) ?></td>
                    <td class="text-end text-success"><?= money($summary['total_received']) ?></td>
                    <td class="text-end text-danger"><?= money($summary['total_outstanding']) ?></td>
                    <td></td>
                </tr>
            </tfoot>
            <?php endif; ?>
...
<?php if ($pagination['total_pages'] > 1): ?>
    <nav class="mt-3 mb-4">
        <?php
        $baseUrl = 'report_sales.php?' . http_build_query(array_diff_key($_GET, ['page' => '']));
