...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        header("Location: party_edit.php?id={$id}");
        exit;
    }
    ...
    query("UPDATE parties SET type=?, name=?, phone=?, email=?, address=?, city=?, state=?, pincode=?, gstin=?, pan=?, gst_reg_type=?, opening_balance=?, party_group=?, notes=?, status=? WHERE id=?", [
        $type, $name, $phone, $email, $address, $city, $state, $pincode, $gstin, $pan, $gst_reg_type, $opening_balance, $party_group, $notes, $statusInt, $id
    ]);

    setFlash('success', 'Party updated successfully.');
    header("Location: party_view.php?id={$id}");
    exit;
}
...
$isActive = ($party['status'] ?? 1) == 1;
$obVal = abs((float)($party['opening_balance'] ?? 0));
$obType = ($party['opening_balance'] ?? 0) < 0 ? 'debit' : 'credit';
$currentType = $party['type'] ?? 'customer';
?>
...
<form method="POST">
    ...
    <div class="vy-add-grid">
        ...
        <div class="vy-add-left">
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-tax-pills" id="typePills">
                        <button type="button" class="vy-pill <?= $currentType === 'customer' ? 'active' : '' ?>" data-val="customer" onclick="setType(this)"><i class="fas fa-user me-1"></i> Customer</button>
                        <button type="button" class="vy-pill <?= $currentType === 'supplier' ? 'active' : '' ?>" data-val="supplier" onclick="setType(this)"><i class="fas fa-truck me-1"></i> Supplier</button>
                        <button type="button" class="vy-pill <?= $currentType === 'both' ? 'active' : '' ?>" data-val="both" onclick="setType(this)"><i class="fas fa-arrows-left-right me-1"></i> Both</button>
                    </div>
                    <input type="hidden" name="type" id="typeInput" value="<?= h($currentType) ?>">
                </div>
            </div>
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        ...
                        <input type="text" name="name" required placeholder="e.g. Rahul Traders" value="<?= h($party['name']) ?>">
                    </div>
                    <div class="vy-f-row">
                        <div class="vy-f">
                            <label>Phone</label>
                            <input type="text" name="phone" placeholder="9876543210" value="<?= h($party['phone'] ?? '') ?>">
                        </div>
                        <div class="vy-f">
                            <label>Email</label>
                            <input type="email" name="email" placeholder="rahul@email.com" value="<?= h($party['email'] ?? '') ?>">
                        </div>
                    </div>
                    <div class="vy-f">
                        <label>Party Group</label>
                        <input type="text" name="party_group" placeholder="e.g. Wholesalers, Retailers" value="<?= h($party['party_group'] ?? '') ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        <label>Address</label>
                        <textarea name="address" rows="2" placeholder="Full address..."><?= h($party['address'] ?? '') ?></textarea>
                    </div>
                    <div class="vy-f-row">
                        <div class="vy-f">
                            <label>City</label>
                            <input type="text" name="city" placeholder="Mumbai" value="<?= h($party['city'] ?? '') ?>">
                        </div>
                        <div class="vy-f">
                            <label>State</label>
                            <input type="text" name="state" placeholder="Maharashtra" value="<?= h($party['state'] ?? '') ?>">
                        </div>
                    </div>
                    <div class="vy-f">
                        <label>Pincode</label>
                        <input type="text" name="pincode" maxlength="6" placeholder="400001" value="<?= h($party['pincode'] ?? '') ?>">
                    </div>
                </div>
            </div>
        ...
        <div class="vy-add-right">
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        ...
                        $selGST = $party['gst_reg_type'] ?? '';
                        ?>
                        <select name="gst_reg_type">
                            <option value="">-- Select --</option>
                            <?php foreach ($gstTypes as $val => $label): ?>
                    ...
                    <div class="vy-f">
                        <label>GSTIN</label>
                        <input type="text" name="gstin" maxlength="15" placeholder="22AAAAA0000A1Z5" value="<?= h($party['gstin'] ?? '') ?>" style="text-transform:uppercase;">
                    </div>
                    <div class="vy-f">
                        <label>PAN</label>
                        <input type="text" name="pan" maxlength="10" placeholder="AAAAA0000A" value="<?= h($party['pan'] ?? '') ?>" style="text-transform:uppercase;">
                    </div>
                </div>
            </div>
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        ...
                        <div class="vy-input-addon">
                            <span>&#8377;</span>
                            <input type="number" step="0.01" min="0" name="opening_balance" value="<?= $obVal ?>">
                        </div>
                    </div>
                    <div class="vy-f">
                        <label>Type</label>
                        <div class="d-flex gap-2" id="balanceTypePills">
                            <button type="button" class="btn btn-sm rounded-pill fw-semibold <?= $obType === 'credit' ? 'btn-success' : 'btn-outline-secondary' ?>" onclick="setBalanceType('credit', this)"><i class="fas fa-arrow-down me-1"></i> You'll Receive</button>
                            <button type="button" class="btn btn-sm rounded-pill fw-semibold <?= $obType === 'debit' ? 'btn-danger' : 'btn-outline-secondary' ?>" onclick="setBalanceType('debit', this)"><i class="fas fa-arrow-up me-1"></i> You'll Pay</button>
                        </div>
                        <input type="hidden" name="balance_type" id="balanceTypeInput" value="<?= h($obType) ?>">
                        <div class="mt-2" style="font-size:11px;color:#888;">
                            <i class="fas fa-info-circle me-1"></i>
                            <span id="balanceHint"><?= $obType === 'debit' ? 'You owe party money (Payable)' : 'Party owes you money (Receivable)' ?></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        <textarea name="notes" rows="3" placeholder="Any additional notes about this party..."><?= h($party['notes'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>
    ...
    <div class="vy-form-actions">
        <div class="d-flex align-items-center gap-3">
            <a href="party_view.php?id=<?= $id ?>" class="btn btn-light btn-lg"><i class="fas fa-xmark me-1"></i> Cancel</a>
            <div class="d-flex align-items-center gap-2 px-3 py-2 rounded" style="background:#f1f3f5;">
                ...
                <div class="form-check form-switch mb-0">
                    <input class="form-check-input" type="checkbox" name="status" value="active" <?= $isActive ? 'checked' : '' ?> onchange="this.value = this.checked ? 'active' : 'inactive'" style="cursor:pointer;">
                </div>
            </div>
        </div>
        <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-check me-1"></i> Update Party</button>
    </div>
</form>
