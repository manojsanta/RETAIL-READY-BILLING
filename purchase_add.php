...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($paidAmount > 0 && !in_array($paymentMethod, ['cash', 'bank', 'upi', 'cheque'])) {
        setFlash('danger', 'Please select a valid payment method (Cash, Bank, UPI, or Cheque) when amount paid is greater than 0.');
        header('Location: purchase_add.php');
        exit;
    }
    if ($paidAmount == 0) {
...
(function() {
    ...
    function createItemRow(index) {
        ...
        tr.innerHTML =
            ...
            '<td class="text-end fw-bold row-tax">0.00</td>' +
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.item-search-dropdown') || e.target.closest('.supplier-search-dropdown') || e.target.closest('.item-search-wrapper')) {
            return;
        }
    ...
    // Form submit validation — all via JS, no HTML5 required blocking submit
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (paidAmt > 0 && !document.querySelector('select[name="payment_method"]').value) {
            errors.push('Please select a payment method.');
        }
        if (errors.length > 0) {
            e.preventDefault();
            showAlert(errors.map(function(er) { return '<p>' + er + '</p>'; }).join(''));
            return;
        }
        ...
        if (payMethod === 'cash' && paidAmt > 0) {
            ...
            if (paidAmt > cashBal) {
                e.preventDefault();
                showAlert('<p>Insufficient cash balance.</p><p>Available: ' + cashBal.toFixed(2) + ' | Required: ' + paidAmt.toFixed(2) + '</p>');
                return;
            }
        }
    });
