...
$old = [
    ...
    'description' => '', 'unit' => 'Pcs', 'purchase_price' => '',
    'sale_price' => '', 'tax_rate_id' => '', 'hsn_code' => '',
    'min_stock' => 10, 'opening_stock' => 0, 'status' => 1,
    'purchase_tax_mode' => 'exclusive', 'sale_tax_mode' => 'exclusive'
];
...
foreach ($categories as $cat) {
    if ($old['category_id'] == $cat['id']) {
        $skuPrefix = strtoupper(substr($cat['name'], 0, 3));
        break;
    }
}
$stmtSku = $pdo->prepare("SELECT COUNT(*) FROM items WHERE sku LIKE ?");
$stmtSku->execute([$skuPrefix . '-%']);
$skuCount = (int)$stmtSku->fetchColumn();
$suggestedSku = $skuPrefix . '-' . str_pad($skuCount + 1, 5, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            ...
            if (empty($errors)) {
                $stmt = $pdo->prepare("INSERT INTO items (name, sku, barcode, category_id, description, unit, purchase_price, purchase_price_with_tax, sale_price, sale_price_with_tax, tax_rate_id, purchase_tax_rate_id, purchase_tax_mode, sale_tax_mode, hsn_code, min_stock, current_stock, opening_stock, image, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
                $stmt->execute([
                    ...
                    $old['opening_stock'], $imageName, $old['status']
                ]);
...
<form method="POST" enctype="multipart/form-data" id="itemForm">
                    ...
                    <div id="stockFields">
                        <div class="vy-f">
                            ...
                            <div class="vy-sku-row">
                                <input type="text" name="sku" placeholder="<?= h($suggestedSku) ?>" value="<?= h($old['sku'] ?: $suggestedSku) ?>">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        ...
                        <div class="vy-input-addon">
                            <span>₹</span>
                            <input type="number" name="purchase_price" id="purchasePrice" step="0.01" min="0" required placeholder="0.00" value="<?= h($old['purchase_price']) ?>">
                        </div>
                    </div>
                    <div class="vy-f">
                    ...
                    <div class="vy-tax-toggle">
                        ...
                        <div class="vy-tax-pills">
                            <button type="button" class="vy-pill active" data-target="purchaseTaxMode" data-val="exclusive" onclick="setTaxMode(this)">Excl.</button>
                            <button type="button" class="vy-pill" data-target="purchaseTaxMode" data-val="inclusive" onclick="setTaxMode(this)">Incl.</button>
                        </div>
            ...
            <div class="vy-card">
                ...
                <div class="vy-card-body">
                    <div class="vy-f">
                        ...
                        <div class="vy-input-addon">
                            <span>₹</span>
                            <input type="number" name="sale_price" id="salePrice" step="0.01" min="0" required placeholder="0.00" value="<?= h($old['sale_price']) ?>">
                        </div>
                    </div>
                    <div class="vy-f">
                    ...
                    <div class="vy-tax-toggle">
                        <span class="vy-tax-label">Tax</span>
                        <div class="vy-tax-pills">
                            <button type="button" class="vy-pill active" data-target="saleTaxMode" data-val="exclusive" onclick="setTaxMode(this)">Excl.</button>
                            <button type="button" class="vy-pill" data-target="saleTaxMode" data-val="inclusive" onclick="setTaxMode(this)">Incl.</button>
                        </div>
    ...
    <div class="vy-form-actions">
        ...
        <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-check me-1"></i> Save Item</button>
    </div>
</form>
...
#profitValue.text-success, .vy-profit-total #profitValue { color: #28a745; }
#profitValue.text-danger { color: #dc3545; }
